...
def handle(msg):
    ...
    if msg.body.type == 'init':
        ...
        # Request each node to lock the key we want to write on
        for dest in quorum:
            send(node_id, dest, type='write_lock_cas', key=msg.body.key, write_id=write_id)
        # Create process status information
        write_waiting.update({write_id: {'msg': msg, 'version': None, 'value': None, 'locked': [], 'total': len(quorum), 'confirmed': 0, 'failed': False, 'cas': msg.body.type == 'cas'}})
        write_id = write_id + 1
        ...
        if value == None:
            kv_store.update({msg.body.key: {'value': None, 'version': None, 'lock': msg.body.write_id}})
        ...
        # If lock is already taken, ignore for CAS msg
        elif value['lock'] != None:
           reply(msg, type='write_no_lock', write_id=msg.body.write_id)
        else:
            # Lock key
            value['lock'] = msg.body.write_id
    ...
    elif msg.body.type == 'write_lock_cas':
        # Retrieve value from storage
        value = kv_store.get(msg.body.key)
        # If value isn't present, lock the key anyway
        if value == None:
            ...
            # Reply that the process locked this key with success
            reply(msg, type='write_lock_ok', write_id=msg.body.write_id, key=msg.body.key, version=None, value=None)
        # ignoring "lock already taken" case for CAs
        else:
            # Lock key
            # --DONT LOCK KEY FOR CAS MSG
            # value['lock'] = msg.body.write_id
            kv_store.update(value)
            # Reply that the process locked this key with success
            reply(msg, type='write_lock_ok', write_id=msg.body.write_id, key=msg.body.key, version=value['version'], value=value['value'])
    ...
    elif msg.body.type == 'write_lock_ok' or msg.body.type == 'write_no_lock':
        # Default error msg and code
        error_code = 11
        error_msg = "Cant get lock"
        # Retrieve process info
        queue = write_waiting[msg.body.write_id]
        # Record that the node has replied to the lock request
        queue['locked'].append(msg.src)
        # If lock wasn't successful
        if msg.body.type == 'write_no_lock':
            # Mark process as failed
            queue['failed'] = True
            # Mark CAS as false to preserve this error message
            queue['cas'] = False
        # If lock was successful, update version and value if those are more recent than the one we have
        elif queue['version'] == None or (msg.body.version != None and msg.body.version > queue['version']):
            queue['version'] = msg.body.version
            queue['value'] = msg.body.value
        ...
        if len(queue['locked']) == queue['total']:
            ...
            # If locking was success, write to quorum nodes
            if not queue['failed']:
            ...
            # Else release all locks and reply to maelstrom
            else:
                for dest in queue['locked']:
                    send(node_id, dest, type='write_lock_release', key=queue['msg'].body.key, write_id=msg.body.write_id)
                reply(queue['msg'], type='error', code=error_code, text=error_msg)
                # Cleanup process info
                # Check why this cleanup breaks code
    ...
    elif msg.body.type == 'write_lock_release':
        # Retrive value from storage
        value = kv_store.get(msg.body.key)
        # Check if process is the owner of the lock, if so unlocks value
        if value['lock'] == msg.body.write_id:
            kv_store.update({msg.body.key: {'value': value['value'], 'version': value['version'], 'lock': None}})
